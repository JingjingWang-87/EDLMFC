...
class AccHistoryPlot(Callback):
    ...
    def plot(self):
        ...
        self.line_train_loss = self.ax.plot(self.train_loss, lw=1.8, color='red', label='Train Loss')
        if self.validate:
            self.line_val_acc = self.ax.plot(self.val_accs, lw=1.8, color='purple', label='Val Loss')
